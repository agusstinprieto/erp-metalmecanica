...
import { X, Printer, User, Building2, Clock, Layers, Hash, ShieldCheck } from 'lucide-react';
...
function InfoCell({ icon: Icon, label, value, valueColor, mono }: {
  ...
  return (
    <div className="bg-slate-50 rounded-lg px-2 py-1.5 flex items-start gap-1.5">
      ...
      <div className="min-w-0">
        ...
        <p className={`text-[9px] font-black truncate leading-none ${mono ? 'font-mono' : ''}`}
...
function BadgeCard({ employee, config }: { employee: Employee; config: any }) {
  const turno = (employee as any).turno_operador ?? '';
  ...
  const hireDate = employee.hire_date
    ...
    : '—';
  const statusLabel = ({
    active: 'Activo', inactive: 'Inactivo', vacation: 'Vacaciones', medical_leave: 'Incapacidad',
  } as any)[employee.status] ?? employee.status;
  const statusColor = employee.status === 'active' ? '#10b981' : '#f59e0b';

  return (
    <div className="w-[280px] rounded-[20px] shadow-2xl overflow-hidden border border-slate-700/40 bg-white print:shadow-none print:border-none">
      <div className="bg-slate-900 flex flex-col items-center justify-center pt-7 pb-5 px-4 relative">
        ...
        {config.logo ? (
          <img src={config.logo} alt={config.brandName} className="h-10 object-contain mb-1"
            onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
        ) : (
          <span className="text-xl font-black tracking-tighter text-white mb-1">{config.brandName || 'MCVILL'}</span>
        )}
      ...
      <div className="mx-3 mb-3 grid grid-cols-2 gap-1.5">
        ...
        <InfoCell icon={Layers}    label="Planta / Célula" value={(employee as any).celula_operador ?? (employee as any).tipo_empleado ?? '—'} />
      ...
      <div className="border-t border-slate-100 flex justify-center py-2 px-2 bg-slate-50">
        <div className="scale-[0.72] origin-center">
          <Barcode value={employee.employee_number ?? employee.id.slice(0, 8)}
            width={1.5} height={38} fontSize={10} background="transparent" />
        </div>
      </div>
...
export const BulkBadgePrintModal: React.FC<Props> = ({ isOpen, onClose, employees }) => {
      ...
      <div className="w-full max-w-lg bg-slate-900 border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 shrink-0">
          <div>
            <h2 className="text-lg font-black text-white tracking-tight uppercase">
              Gafetes <span className="text-blue-500">Corporativos</span>
            </h2>
            <p className="text-[9px] font-bold text-slate-500 uppercase tracking-[0.25em] mt-0.5">
              {count} gafete{count !== 1 ? 's' : ''} · {count} página{count !== 1 ? 's' : ''} de impresión
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="w-full bg-blue-500/10 border border-blue-500/20 rounded-xl px-4 py-3 text-[9px] text-blue-300 font-bold space-y-1">
              <p>🖨️ Se generarán <strong>{count} página{count !== 1 ? 's'  : ''}</strong> — una tarjeta por página (85×135 mm)</p>
              <p>💡 En el diálogo de impresión, selecciona tu <strong>impresora de tarjetas plásticas</strong> y desmarca "ajustar al área de impresión"</p>
            </div>

            <div id="bulk-badge-print-container" className="flex flex-col items-center gap-6">
              {employees.map((emp, i) => (
                <div key={emp.id} className={`badge-page-wrapper${i < count - 1 ? ' badge-page-break' : ''}`}>
                  <BadgeCard employee={emp} config={config} />
                </div>
              ))}
            </div>
            ...
            <button onClick={handlePrint}
              ...
              <Printer size={15} /> Imprimir {count} gafete{count !== 1 ? 's' : ''}
            </button>
          </div>
        </div>
      </div>
